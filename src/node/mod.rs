...
use node_bindgen::{core::{val::JsObject, NjError, TryIntoJs}, derive::node_bindgen};
use num_enum::TryFromPrimitive;
use serde::Serialize;
use serde_json::Value;
// use tslink::tslink;
use crate::{edupage::{Edupage as IEdupage, EdupageError}, edupage_traits::{Login, Ringing, Timeline, Timetable, DBI}, edupage_types::{DBIBase, RingingTime, Student, Teacher, TimelineItem, TimelineItemType, Timetable as EduTimetable, UserData}};


struct SerdeIntoJs<T: Serialize>(T);

impl<T: Serialize> SerdeIntoJs<T> {
    fn value_to_js(js_env: &node_bindgen::core::val::JsEnv, value: Value) -> Result<node_bindgen::sys::napi_value, node_bindgen::core::NjError> {
        match value {
...
impl<T: Serialize> TryIntoJs for SerdeIntoJs<T> {
    ...
    fn try_to_js(self, js_env: &node_bindgen::core::val::JsEnv) -> Result<node_bindgen::sys::napi_value, node_bindgen::core::NjError> {
        let serialized_instance: Value = serde_json::to_value(self.0).map_err(|e| NjError::Other(e.to_string()))?;
...
impl Edupage {
    ...
    #[node_bindgen]
    pub fn login(&mut self, subdomain: String, username: String, password: String) -> Result<(), SerdeIntoJs<EdupageError>> {
        self.0.login(&subdomain, &username, &password).map_err(|e| SerdeIntoJs(e))
    }
    ...
    #[node_bindgen]
    pub fn get_teacher_by_id(&self, id: i64) -> Result<Option<SerdeIntoJs<Teacher>>, SerdeIntoJs<EdupageError>> {
        self.0.get_teacher_by_id(id)
            .map(|r| {
                match r {
    ...
    #[node_bindgen]
    pub fn get_student_by_id(&self, id: i64) -> Result<Option<SerdeIntoJs<Student>>, SerdeIntoJs<EdupageError>> {
        self.0.get_student_by_id(id)
            .map(|r| {
                match r {
                    Some(v) => Some(SerdeIntoJs(v)),
                    None => None
    ...
    #[node_bindgen]
    pub fn get_subject_by_id(&self, id: i64) -> Result<Option<SerdeIntoJs<DBIBase>>, SerdeIntoJs<EdupageError>> {
        self.0.get_subject_by_id(id)
            .map(|r| {
                match r {
                    Some(v) => Some(SerdeIntoJs(v)),
                    None => None
    ...
    #[node_bindgen]
    pub fn get_classroom_by_id(&self, id: i64) -> Result<Option<SerdeIntoJs<DBIBase>>, SerdeIntoJs<EdupageError>> {
        self.0.get_classroom_by_id(id)
            .map(|r| {
                match r {
                    Some(v) => Some(SerdeIntoJs(v)),
                    None => None
    ...
    #[node_bindgen]
    pub fn filter_timeline_by_item_type(&self, item_type: i32) -> Result<Vec<SerdeIntoJs<TimelineItem>>, SerdeIntoJs<EdupageError>> {
        let item_type = match TimelineItemType::try_from_primitive(item_type as usize) {
            Ok(v) => v,
            Err(_) => return Err(
                SerdeIntoJs(
                    EdupageError::ParseError("Invalid item type!".to_string())
                )
            )
        };

        self.0.filter_timeline_by_item_type(item_type)
            .map(|r| r
                .into_iter()
                .map(|k| SerdeIntoJs(k))
                .collect()
    ...
    #[node_bindgen]
    pub fn filter_by_item_types(&self, item_types: Vec<i32>) -> Result<Vec<SerdeIntoJs<TimelineItem>>, SerdeIntoJs<EdupageError>> {
        ...
        self.0.filter_timeline_by_item_types(typed_item_types)
            .map(|r| r
                .into_iter()
                .map(|k| SerdeIntoJs(k))
                .collect()
    ...
    #[node_bindgen]
    pub fn get_timetable(&self, date_timestamp: i64) -> Result<SerdeIntoJs<EduTimetable>, SerdeIntoJs<EdupageError>> {
        let datetime = Utc.timestamp_opt(date_timestamp, 0).unwrap().naive_local();

        self.0.get_timetable(datetime.date())
            .map(|r| SerdeIntoJs(r))
            .map_err(|e| SerdeIntoJs(e))
    }
